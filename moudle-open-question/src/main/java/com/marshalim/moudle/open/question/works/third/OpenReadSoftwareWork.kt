...
import putAppThirdSoftwareData

/**
 * 读取软件题库
 */
class OpenReadSoftwareWork : Runnable {
        ...
        for ((index, itemCell) in arrayZero.withIndex()) {
            ...
            if ((itemCell.contents?.contentEquals("题型") == false)) {
                ...
                if (itemCell.contents.equals("单选题")) {
                    bean.questionType = 1
                } else if (itemCell.contents.equals("判断题")) {
                    bean.questionType = 3
                } else if (itemCell.contents.equals("多选题")) {
                    bean.questionType = 2
                }
                arrayList.add(bean)
            }
