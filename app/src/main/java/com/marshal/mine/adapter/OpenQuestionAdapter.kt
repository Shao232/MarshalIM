...
class OpenQuestionAdapter : BaseRecyclerAdapter<OpenQuestionViewHolder, OpenAnswersBean>() {
    ...
    override fun bindViewHolderData(holder: OpenQuestionViewHolder, position: Int) {
        ...
        if (selectSelfAnswerBean != null) {
            ...
            when (openAnswersBean.questionType) {
                1 -> {
            ...
            val hasRight = if (openAnswersBean.questionType == 1 || openAnswersBean.questionType == 2) {
                openAnswersBean.rightAnswer == selectSelfAnswerBean.answerTitle
            } else {
                openAnswersBean.rightAnswer == selectSelfAnswerBean.answerContent
            }
        ...
        adapter.setOnSelectAnswerClickListener(object :
            ...
            override fun onSelectAnswer(view: View, bean: OpenAnswerBean) {
                if (bean.hasSelectSelf == true) {
                    ...
                    val hasRight = if (openAnswersBean.questionType == 1 || openAnswersBean.questionType == 2) {
                        openAnswersBean.rightAnswer == bean.answerTitle
                    } else {
                        openAnswersBean.rightAnswer == bean.answerContent
                    }
